...
if manualReset != lastManualReset and manualReset
    resetBacktestArrays()
    resetCount := resetCount + 1
...
processSignalBacktest(signalIndex, longCondition, shortCondition) =>
    if signalIndex >= 0 and signalIndex < 5
        ...
        // ENTRY LOGIC: Only enter when signal fires (simplified for backtesting)
...
// ═══════════════════ STRATEGY-REALISTIC SIGNAL BACKTEST TABLE ═══════════════════
// Shows performance as if each signal was the only active signal using actual strategy logic
if showBacktestTable and barstate.isconfirmed
    // Create backtesting comparison table with drawdown data
    var table backtestTable = table.new(position.middle_left, 7, 7, bgcolor = color.new(color.black, 10), border_width = 2)

    // Headers with high contrast - Dollar amounts for P&L and DD
    table.cell(backtestTable, 0, 0, '📊 STRATEGY-REALISTIC BACKTEST', text_color = color.white, text_size = size.normal, bgcolor = color.new(color.blue, 20))
    table.cell(backtestTable, 1, 0, 'Win Rate %', text_color = color.white, text_size = size.small, bgcolor = color.new(color.green, 20))
    table.cell(backtestTable, 2, 0, 'Net P&L $', text_color = color.white, text_size = size.small, bgcolor = color.new(color.green, 20))
    table.cell(backtestTable, 3, 0, 'Max DD $', text_color = color.white, text_size = size.small, bgcolor = color.new(color.red, 20))
    table.cell(backtestTable, 4, 0, 'Profit Factor', text_color = color.white, text_size = size.small, bgcolor = color.new(color.blue, 20))
    table.cell(backtestTable, 5, 0, 'Trades', text_color = color.white, text_size = size.small, bgcolor = color.new(color.blue, 20))
    table.cell(backtestTable, 6, 0, '⭐ Rating', text_color = color.white, text_size = size.small, bgcolor = color.new(color.blue, 20))

    // Signal 1 data with STRATEGY-REALISTIC backtesting
    if signal1Enable
        [netProfit1, maxDD1, profitFactor1, totalTrades1, winRate1, currentDD1] = getSignalResults(0)

        // Calculate rating based on comprehensive metrics
        rating1 = (winRate1 > 0.6 and netProfit1 > 0 and maxDD1 < 10 and profitFactor1 > 1.5) ? '⭐⭐⭐' : 
                 (winRate1 > 0.5 and netProfit1 > 0 and maxDD1 < 15 and profitFactor1 > 1.2) ? '⭐⭐' : 
                 (winRate1 > 0.4 and netProfit1 > 0) ? '⭐' : '❌'
        ratingColor1 = rating1 == '⭐⭐⭐' ? color.lime : rating1 == '⭐⭐' ? color.yellow : rating1 == '⭐' ? color.orange : color.red

        table.cell(backtestTable, 0, 1, signal1Name, text_color = color.white, text_size = size.small, bgcolor = color.new(color.gray, 80))
        ...
        table.cell(backtestTable, 2, 1, '$' + str.tostring(netProfit1, '#.##'), text_color = netProfit1 > 0 ? color.lime : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 3, 1, '$' + str.tostring(math.abs(maxDD1), '#.##'), text_color = maxDD1 < -100 ? color.lime : maxDD1 < -500 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 4, 1, str.tostring(profitFactor1, '#.##'), text_color = profitFactor1 > 1.5 ? color.lime : profitFactor1 > 1.2 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 5, 1, str.tostring(totalTrades1, '#'), text_color = color.white, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 6, 1, rating1, text_color = ratingColor1, text_size = size.small, bgcolor = color.new(color.gray, 90))

    // Signal 2 data with STRATEGY-REALISTIC backtesting
    if signal2Enable
        [netProfit2, maxDD2, profitFactor2, totalTrades2, winRate2, currentDD2] = getSignalResults(1)
        
        rating2 = (winRate2 > 0.6 and netProfit2 > 0 and maxDD2 < 10 and profitFactor2 > 1.5) ? '⭐⭐⭐' : 
                 (winRate2 > 0.5 and netProfit2 > 0 and maxDD2 < 15 and profitFactor2 > 1.2) ? '⭐⭐' : 
                 (winRate2 > 0.4 and netProfit2 > 0) ? '⭐' : '❌'
        ratingColor2 = rating2 == '⭐⭐⭐' ? color.lime : rating2 == '⭐⭐' ? color.yellow : rating2 == '⭐' ? color.orange : color.red
        ...
        table.cell(backtestTable, 2, 2, '$' + str.tostring(netProfit2, '#.##'), text_color = netProfit2 > 0 ? color.lime : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 3, 2, '$' + str.tostring(math.abs(maxDD2), '#.##'), text_color = maxDD2 < -100 ? color.lime : maxDD2 < -500 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 4, 2, str.tostring(profitFactor2, '#.##'), text_color = profitFactor2 > 1.5 ? color.lime : profitFactor2 > 1.2 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 5, 2, str.tostring(totalTrades2, '#'), text_color = color.white, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 6, 2, rating2, text_color = ratingColor2, text_size = size.small, bgcolor = color.new(color.gray, 90))

    // Signal 3 data with STRATEGY-REALISTIC backtesting
    if signal3Enable
        [netProfit3, maxDD3, profitFactor3, totalTrades3, winRate3, currentDD3] = getSignalResults(2)
        
        rating3 = (winRate3 > 0.6 and netProfit3 > 0 and maxDD3 < 10 and profitFactor3 > 1.5) ? '⭐⭐⭐' : 
                 (winRate3 > 0.5 and netProfit3 > 0 and maxDD3 < 15 and profitFactor3 > 1.2) ? '⭐⭐' : 
                 (winRate3 > 0.4 and netProfit3 > 0) ? '⭐' : '❌'
        ratingColor3 = rating3 == '⭐⭐⭐' ? color.lime : rating3 == '⭐⭐' ? color.yellow : rating3 == '⭐' ? color.orange : color.red

        table.cell(backtestTable, 0, 3, signal3Name, text_color = color.white, text_size = size.small, bgcolor = color.new(color.gray, 80))
        table.cell(backtestTable, 1, 3, str.tostring(winRate3 * 100, '#.#') + '%', text_color = winRate3 > 0.6 ? color.lime : winRate3 > 0.5 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 2, 3, '$' + str.tostring(netProfit3, '#.##'), text_color = netProfit3 > 0 ? color.lime : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 3, 3, '$' + str.tostring(math.abs(maxDD3), '#.##'), text_color = maxDD3 < -100 ? color.lime : maxDD3 < -500 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 4, 3, str.tostring(profitFactor3, '#.##'), text_color = profitFactor3 > 1.5 ? color.lime : profitFactor3 > 1.2 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 5, 3, str.tostring(totalTrades3, '#'), text_color = color.white, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 6, 3, rating3, text_color = ratingColor3, text_size = size.small, bgcolor = color.new(color.gray, 90))

    // Signal 4 data with STRATEGY-REALISTIC backtesting
    if signal4Enable
        [netProfit4, maxDD4, profitFactor4, totalTrades4, winRate4, currentDD4] = getSignalResults(3)
        
        rating4 = (winRate4 > 0.6 and netProfit4 > 0 and maxDD4 < 10 and profitFactor4 > 1.5) ? '⭐⭐⭐' : 
                 (winRate4 > 0.5 and netProfit4 > 0 and maxDD4 < 15 and profitFactor4 > 1.2) ? '⭐⭐' : 
                 (winRate4 > 0.4 and netProfit4 > 0) ? '⭐' : '❌'
        ratingColor4 = rating4 == '⭐⭐⭐' ? color.lime : rating4 == '⭐⭐' ? color.yellow : rating4 == '⭐' ? color.orange : color.red

        table.cell(backtestTable, 0, 4, signal4Name, text_color = color.white, text_size = size.small, bgcolor = color.new(color.gray, 80))
        table.cell(backtestTable, 1, 4, str.tostring(winRate4 * 100, '#.#') + '%', text_color = winRate4 > 0.6 ? color.lime : winRate4 > 0.5 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 2, 4, '$' + str.tostring(netProfit4, '#.##'), text_color = netProfit4 > 0 ? color.lime : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 3, 4, '$' + str.tostring(math.abs(maxDD4), '#.##'), text_color = maxDD4 < -100 ? color.lime : maxDD4 < -500 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 4, 4, str.tostring(profitFactor4, '#.##'), text_color = profitFactor4 > 1.5 ? color.lime : profitFactor4 > 1.2 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 5, 4, str.tostring(totalTrades4, '#'), text_color = color.white, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 6, 4, rating4, text_color = ratingColor4, text_size = size.small, bgcolor = color.new(color.gray, 90))

    // Signal 5 data with STRATEGY-REALISTIC backtesting
    if signal5Enable
        [netProfit5, maxDD5, profitFactor5, totalTrades5, winRate5, currentDD5] = getSignalResults(4)
        
        rating5 = (winRate5 > 0.6 and netProfit5 > 0 and maxDD5 < 10 and profitFactor5 > 1.5) ? '⭐⭐⭐' : 
                 (winRate5 > 0.5 and netProfit5 > 0 and maxDD5 < 15 and profitFactor5 > 1.2) ? '⭐⭐' : 
                 (winRate5 > 0.4 and netProfit5 > 0) ? '⭐' : '❌'
        ratingColor5 = rating5 == '⭐⭐⭐' ? color.lime : rating5 == '⭐⭐' ? color.yellow : rating5 == '⭐' ? color.orange : color.red

        table.cell(backtestTable, 0, 5, signal5Name, text_color = color.white, text_size = size.small, bgcolor = color.new(color.gray, 80))
        table.cell(backtestTable, 1, 5, str.tostring(winRate5 * 100, '#.#') + '%', text_color = winRate5 > 0.6 ? color.lime : winRate5 > 0.5 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 2, 5, '$' + str.tostring(netProfit5, '#.##'), text_color = netProfit5 > 0 ? color.lime : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 3, 5, '$' + str.tostring(math.abs(maxDD5), '#.##'), text_color = maxDD5 < -100 ? color.lime : maxDD5 < -500 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 4, 5, str.tostring(profitFactor5, '#.##'), text_color = profitFactor5 > 1.5 ? color.lime : profitFactor5 > 1.2 ? color.yellow : color.red, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 5, 5, str.tostring(totalTrades5, '#'), text_color = color.white, text_size = size.small, bgcolor = color.new(color.gray, 90))
        table.cell(backtestTable, 6, 5, rating5, text_color = ratingColor5, text_size = size.small, bgcolor = color.new(color.gray, 90))
